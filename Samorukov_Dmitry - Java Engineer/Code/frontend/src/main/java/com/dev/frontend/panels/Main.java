...
public class Main implements PanelSwitcher {
	...
	public Main() {
		try {
			...
			properties.load(getClass().getClassLoader().getResourceAsStream("frontend.properties"));
			BACKEND_API_URL = properties.getProperty("backend.url.api");
		} catch (Exception e) {
			BACKEND_API_URL = "http://localhost:8080/backend/api";
		}
		initialize();
	}
